...
unique(fungi$BroadSymbiontGroup)
...
p2_fg <- ggplot(guild, aes(x = 1, y = n, fill = all_functional)) +
  geom_col(width = 0.75) +
  coord_flip() +
  scale_fill_manual(values = pal_tableau10, drop = FALSE) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.02))) +
...
count(fungi, host_substrate)
str(fungi)
guild_prop <- fungi %>%
  filter(!is.na(BroadSymbiontGroup)) %>%
  mutate(
    habitat = dplyr::recode(host_substrate, `E` = "Epiphytic", `T` = "Terrestrial"),
    BroadSymbiontGroup = factor(BroadSymbiontGroup, levels = broad_symbionts)
  ) %>%
  distinct(ASV, habitat, BroadSymbiontGroup) %>%
  count(habitat, BroadSymbiontGroup) %>%
  group_by(habitat) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()
...
p_guild <- ggplot(guild_prop,
                  aes(x = habitat, y = prop, fill = BroadSymbiontGroup)) +
  coord_flip() +
  geom_col(width = 0.7) +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_fill_manual(values = pal_tableau10, drop = FALSE) +
...
guild_prop_fam <- fungi %>%
  filter(!is.na(BroadSymbiontGroup)) %>%
  mutate(
    BroadSymbiontGroup = factor(BroadSymbiontGroup, levels = broad_symbionts),
    host_family = factor(host_family, levels = c("Araceae","Bromeliaceae","Orchidaceae","Piperaceae"))
  ) %>%
  distinct(ASV, host_family, BroadSymbiontGroup) %>%     # ASV-level composition per family
  count(host_family, BroadSymbiontGroup, name = "n") %>%
  group_by(host_family) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()

p_family <- ggplot(guild_prop_fam,
                         aes(x = host_family, y = prop, fill = BroadSymbiontGroup)) +
  coord_flip() +
  geom_col(width = 0.7) +
  scale_y_continuous(labels = scales::percent_format()) +
  ...
  labs(
    ...
    x = NULL,
    fill = NULL,
    title = "Symbiotic functional guild composition by host family"
  ) +
  theme_classic(base_size = 11) +
  theme(
    axis.text.x = element_text(size = base_axis_size),
...
guild_prop_fam_hab <- fungi %>%
  ...
  mutate(
    habitat = dplyr::recode(host_substrate, `E` = "Epiphytic", `T` = "Terrestrial"),
    habitat = factor(habitat, levels = c("Epiphytic", "Terrestrial")),
    BroadSymbiontGroup = factor(BroadSymbiontGroup, levels = broad_symbionts),
    host_family = factor(host_family, levels = c("Araceae","Bromeliaceae","Orchidaceae","Piperaceae"))
  ) %>%
  distinct(ASV, host_family, habitat, BroadSymbiontGroup) %>%  # ASV-level composition per family×habitat
  count(host_family, habitat, BroadSymbiontGroup, name = "n") %>%
  group_by(host_family, habitat) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()

p_guild_family_habitat <- ggplot(guild_prop_fam_hab,
                                 aes(x = habitat, y = prop, fill = BroadSymbiontGroup)) +
  geom_col(width = 0.7) +
  ...
  labs(
    ...
    x = NULL,
    fill = NULL,
    title = "Symbiotic guild composition by habitat within host families"
  ) +
  theme_classic(base_size = 15) +
  theme(
    axis.text.x = element_text(size = base_axis_size),
...
ggsave(
  filename = "../Figures/Figure4.barplots_guild.png",
  plot = guild_combined,
  width = 13,
  height = 7,
  dpi = 600,
...
forest_dat <- forest_dat %>%
  ...
  mutate(
    direction = if_else(IRR > 1, "Terrestrial > Epiphytic", "Epiphytic > Terrestrial")
  )
...
forest_plot_guilds <- ggplot(
  ...
  geom_vline(xintercept = 1, linetype = "dashed", linewidth = 0.6) +
  geom_errorbarh(aes(xmin = CI_low, xmax = CI_high), height = 0.05, linewidth = 0.9) +
  geom_point(size = 3.5) +
  scale_x_log10(
    breaks = c(0.5, 0.75, 1, 2.5, 5),
...
forest_fam <- imap_dfr(fits_nb, function(model, guild) {
  ...
  mutate(
    # enforce correct y-order + stable labeling
    guild = factor(guild, levels = broad_symbionts),
    
    # optional: order families consistently across legend/colors
...
forest_plot_family_guilds <- ggplot( forest_fam, aes(x = IRR, y = guild, color = family) ) +
  geom_vline(xintercept = 1, linetype = "dashed", linewidth = 0.6) + 
  geom_errorbarh( aes(xmin = CI_low, xmax = CI_high, alpha = alpha_val), 
      height = 0.18, linewidth = 0.9, position = position_dodge(width = 0.8) ) +
  geom_point( aes(alpha = alpha_val), size = 3.2, position = position_dodge(width = 0.8) ) +
  scale_x_log10(breaks = c(0.5, 0.75, 1, 1.25, 2.5, 5),
    labels = c("0.5","0.75","1","1.25","2.5","5")
  ) +
...
ggsave(
  filename = "../Figures/Figure5.all_forest_plots.png",
